...
class WayPointPlotter:
    ...
    def init_waypoint_markers(self):
        ...
        self.id = 0
        marker_id = 0
        marker_line_color = {'r': 0.0, 'g': 1.0, 'b': 0.0, 'a': 1.0}
        marker_circle_color = {'r': 0.0, 'g': 0.0, 'b': 1.0, 'a': 1.0}
        
        self.waypoint_marker_array = MarkerArray()
        #self.waypoint_marker_array.
        ...
        self.waypoint_markers_line.scale.x = marker_scale
        self.waypoint_markers_line.scale.y = marker_scale
        ...
        self.waypoint_markers_circle.scale.x = 0.3
    ...
    def import_param_csv(self, path):
        self.waypoints = []
        self.waypoints_name = []
        print ('path', path)
    ...
    def add_waypoint_cb(self,data):
        #print data
        self.waypoints.append(data.pose.pose)
        ...
        param_str = 'name,x,y,z,qx,qy,qz,qw\n'
    ...
    def __init__(self):
        rospy.init_node('waypoint_list_plotter', anonymous=False)
        rate = rospy.Rate(1)
        self.csv_path = rospy.get_param('~csv_path',default='/home/minirw/amv_ws/src/clmr_connect/waypoints/path_waypoints_list.csv')
        ...
        self.waypoint_marker_array = MarkerArray()
        way_sub = rospy.Subscriber("/initialpose_data", PoseWithCovarianceStamped, self.add_waypoint_cb) 
        while not rospy.is_shutdown(): 
            self.marker_pub.publish(self.waypoint_marker_array)
            rate.sleep()
        rospy.loginfo('Waypoint plotter terminated')       
