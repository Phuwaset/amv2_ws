...
class amv_base_node(BaseConnect):
    def __init__(self):
        ...
        self.csv_path_logOdom = rospy.get_param('~LogOdom_path', default='/home/minirw/amv_ws/src/amv_connect/src/Log_Odom.csv')
        self.csv_path_logEncode = rospy.get_param('~LogEncode_path', default='/home/minirw/amv_ws/src/amv_connect/src/Log_Encode.csv')
    ...
    def odom_publish(self, data):
        ...
        if self.amv_linear_velocity > 0 or self.amv_angular_velocity > 0 :
            now = datetime.now()
            dt_string = now.strftime("%d/%m/%Y %H:%M:%S")  
            log_str = dt_string + " => x_odom : " + str(self.x_odom) + " , y_odom : " + str(self.y_odom) + "\n" 
            logEn_str = dt_string + " => Encoder Linear : " + str(self.amv_linear_velocity) + " , Encoder Angular : " + str(self.amv_angular_velocity) + "\n"
            #with open(self.csv_path_logOdom, 'a') as logOdom:
            #    logOdom.write(log_str)
            #with open(self.csv_path_logEncode, 'a') as logEncode:
            #    logEncode.write(logEn_str)
