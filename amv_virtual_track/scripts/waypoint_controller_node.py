        ...
        while not rospy.is_shutdown():
            ...
            # -------------------------------------------------------------
                ...
                if at_entry:
                    ...
                    # โซนวิ่งตรง: 0.30 ม. สุดท้ายก่อนหยุด -> ล็อกหัวหุ่น (w=0) ไม่ใช้ Pure Pursuit
                    # (entry_mode อิงระยะถึงจุดเช็ค เพราะหยุดก่อนสถานี; เคสปกติอิงระยะถึงสถานี)
                    stop_dist_ref = dist_entry if entry_mode else dist_to_goal
                    straight_zone = stop_dist_ref <= self.decel_dist_station
                    if straight_zone:
                        rospy.loginfo_throttle(1.0, f"[RPP] Straight zone: {stop_dist_ref:.2f} m to stop, steering locked (w=0).")
                    ...
                    # 2. RPP Approach Deceleration (ยิ่งใกล้เป้า ยิ่งต้องช้า; อิงระยะจุดที่จะหยุด)
                    v_approach = self.max_lin_vel
                    if stop_dist_ref < self.decel_dist_station:
                        v_approach = self.min_lin_vel + (self.max_lin_vel - self.min_lin_vel) * (stop_dist_ref / self.decel_dist_station)
                    ...
                    if straight_zone:
                        # โซนวิ่งตรง: ไม่ผูกกับ v_curve (ไม่มี steering ตามความโค้ง)
                        target_v = max(self.min_lin_vel, v_approach)
                    else:
                    ...
                    if straight_zone:
                        # โซนวิ่งตรง: ล็อกหัวหุ่นปัจจุบัน ไม่หมุนเลย (w=0)
                        cmd.angular.z = 0.0
                    else:
