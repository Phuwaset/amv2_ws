...
class WayPointTracking():
    ...
    def follow_waypoint(self):
        ...
        error = self.find_direction(self.waypoints[self.target_waypoint_no])
        ...
        if(abs(error[0]) < 90) and (abs(error[2]) < 90):
            ...
            pose_ind_list.append(self.target_waypoint_no)

        #rospy.loginfo("heading_err, distance_err, point_err: {0} {1} {2}".format(error[0], error[1], error[2]))
        print ('-----------------------------------------------')
        print ('Go to waypoint: ', self.target_waypoint_no+1)  
        print (self.waypoints[self.target_waypoint_no])

        goal = MoveBaseGoal()
        goal.target_pose.header.frame_id = self.frame_id
        goal.target_pose.pose.position = self.waypoints[self.target_waypoint_no].position
        goal.target_pose.pose.orientation = self.waypoints[self.target_waypoint_no].orientation
        rospy.loginfo('Executing move_base goal to position (x,y): %s, %s' %(self.waypoints[self.target_waypoint_no].position.x, self.waypoints[self.target_waypoint_no].position.y))
        ...
        if not self.distance_tolerance > 0.0:
            ...
            if (self.waypoints_mode[self.target_waypoint_no] == 'A'):             
                rospy.loginfo("Waiting for delay %d second" % self.waypoints_timer[self.target_waypoint_no])
                time.sleep(self.waypoints_timer[self.target_waypoint_no])
            elif (self.waypoints_mode[self.target_waypoint_no] == 'M'):
            ...
            while(distance > self.distance_tolerance):
                ...
                distance = math.sqrt(pow(self.waypoints[self.target_waypoint_no].position.x-trans[0],2)+pow(self.waypoints[self.target_waypoint_no].position.y-trans[1],2))
    ...
    def target_waypoint_callback(self,data):
        self.target_waypoint_no = int(data.data)
    ...
    def __init__(self):
        ...
        self.amv_waypoint_sub = rospy.Subscriber("target_waypoint_no", String, self.target_waypoint_callback)
