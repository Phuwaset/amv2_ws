...
class QrPoseStandalone():    
    ...
    def tag_callback(self, tag_data):
        ...
        if i > 0:
            ...
            if tag_id == self.previous_tag:                
                self.tag_detect = '/tag' + str(tag_id)
                #self.qr_count += 1
            else:
                self.qr_count = 0  
            self.previous_tag = tag_id
        else:
    ...
    def qr_distance_publish(self):
        ...
        self.qr_distance = math.sqrt(pow(trans[0],2) + pow(trans[1],2))

    def qr_set_position(self):
        for i in range(0, len(self.landmark_list)):
            if self.tag_detect == '/' + self.landmark_list[i].tag_id: 
                ...
                print (rotation_frame, rotation_frame * 180 /math.pi)
                ...
                B = np.array([[math.cos(rotation_frame), -math.sin(rotation_frame), 0, px_landmark],
                            [math.sin(rotation_frame),  math.cos(rotation_frame), 0, py_landmark],
                            [0,                                 0,                  1, 0],
                            [0,                                 0,                  0, 1]])

                vector_amv = np.dot(B,A)

                px_amv = vector_amv[0]
                py_amv = vector_amv[1]
                angle_amv = rotation_frame   

                q_new = tf.transformations.quaternion_from_euler(0, 0, angle_amv)

                reset_position = PoseWithCovarianceStamped()
                reset_position.header.frame_id = 'map'
                reset_position.pose.pose.position.x = px_amv
                reset_position.pose.pose.position.y = py_amv
    ...
    def __init__(self):
        ...
        self.qr_to_base_pub = rospy.Publisher('/qr_to_basefootprint', PoseStamped, queue_size=1)
        ...
        while not rospy.is_shutdown():           
            try:
                if (self.tag_detect != 'None'):
                    ...
                    if self.qr_distance < self.qr_set_distance:
                        self.qr_set_position()
