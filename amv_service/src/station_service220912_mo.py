...
class StationService():  
    ...
    def amv_pose_callback(self,data):
        ...
        #self.current_angle = 2 * math.acos(self.current_pose.orientation.w) 

        self.current_angle_tf = euler_from_quaternion(self.quanarr,axes='sxyz')
        self.current_angle = self.current_angle_tf[2]
        #if self.current_pose.orientation.z > 0 :
        #    self.current_angle = self.current_angle * 1.0
        #else :
        #    self.current_angle = self.current_angle * (-1.0)
    ...
    def path_callback(self,data):
        if len(data.poses) > 10 :
            twist = Twist()
            path_x = data.poses[10].pose.position.x
            path_y = data.poses[10].pose.position.y
            current_x = self.current_pose.position.x
            current_y = self.current_pose.position.y

            self.cal_diff_x = path_x - current_x
            self.cal_diff_y = path_y - current_y
            ...
            if self.st_cal_ang == False :
                ...
                if (self.current_angle >= 0 and self.current_angle <= 45) or (self.current_angle >= 315 and self.current_angle <= 359):
                    if self.cal_diff_x < 0:
                        self.cal_angle_deg =  self.cal_angle_deg * (-1)
                    else:
                        self.cal_angle_deg =  self.cal_angle_deg * (1)

                elif (self.current_angle > 45 and self.current_angle <= 135):
                    if self.cal_diff_y < 0:
                        self.cal_angle_deg =  self.cal_angle_deg * (-1)
                    else:
                        self.cal_angle_deg =  self.cal_angle_deg * (1)
                ...
                self.mov_angle = self.current_angle + self.cal_angle_deg
                

                if self.mov_angle >= 360:
                    self.mov_angle = self.mov_angle -360
                else :
                    self.mov_angle = self.mov_angle 

                self.diff_angle = self.current_angle + self.cal_angle_deg
                

            if self.start_rotate == False:

                if self.mov_angle > 1.2  or  self.mov_angle < -1.2 and self.check_diff_angle == False:
                    self.check_diff_angle = True
                if self.check_diff_angle == True:
                    if self.mov_angle > self.current_angle:
                        twist.linear.x = 0
                        twist.angular.z = 0.1
                        print ('ccw')
